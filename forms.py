...
from wtforms import StringField, SubmitField, PasswordField, BooleanField, SelectField, TextAreaField, TelField, EmailField, DecimalField
...
class ContactUsForm(FlaskForm):
    ...
    phone_number = TelField("Phone number", validators=[Regexp(regex=r"^((\+[0-9]{0,4}) [0-9]{4,13})?$", message="Invalid input. Example: +44 1234567899")])
...
class RegisterForm(FlaskForm):
    email = StringField("Email", validators=[DataRequired(), Email(), Length(min=2, max=50)])
    password = PasswordField("Password", validators=[DataRequired(), Length(min=8, max=50), EqualTo("password_confirm", message="Passwords must match")])
